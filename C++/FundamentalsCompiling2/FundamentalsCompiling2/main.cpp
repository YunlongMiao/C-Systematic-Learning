...
char Anstack[20];		//分析栈
char Rmstring[20];	//剩余串 
char vt[20] = { 'i','+','*','(',')','#' };	//终结符  
char vn[20] = { 'E', 'G', 'T', 'S', 'F'};		//非终结符   
...
int main()
{
	...
	char ch, x;
	type cha;//用来接受LL1table[m][n]  //把文法产生式赋值结构体 
	e.origin = 'E';
	strcpy(e.array, "TG");
	e.length = 2;
	t.origin = 'T';
	...
	for (m = 0; m <= 4; m++)//初始化分析表 
		for (n = 0; n <= 5; n++)
			LL1table[m][n].origin = 'N';//全部赋为空  //填充分析表 
	LL1table[0][0] = e;
	LL1table[0][3] = e;
	LL1table[1][1] = g;
	LL1table[1][4] = g1;
	...
	l = j;//分析串长度 
	ch = Rmstring[0];	//当前分析字符 
	Anstack[top] = '#';
	Anstack[++top] = 'E';	//＇＃＇，＇E＇进栈 
	printf("步骤\t\t分析栈\t\t剩余字符\t\t所用产生式\n");
	do
	{
		x = Anstack[top--];//x为当前栈顶字符  
		printf("%d", k++);
		printf("\t\t");
		...
		else//非终结符处理 
		{
			...
			if (cha.origin != 'N')//判断是否为空  
			{
				...
				for (j = (cha.length - 1); j >= 0; j--)//产生式逆序入栈  
					Anstack[++top] = cha.array[j];
				if (Anstack[top] == '^')//为空则不进栈  
					top--;
			}/*if*/
			else//出错处理 
			{
